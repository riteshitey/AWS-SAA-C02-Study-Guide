...
Ever missed tagging your AWS resources with the AWS discount tag? No worries, we've got the perfect solution for you! 💡
...
- **Comprehensive Coverage:** Tag all resources of a specific AWS service while excluding any you wish. Supports S3, Lambda, Glue, Kinesis, Step Functions, and many more. 🌐

We've used this utility to optimize AWS costs across various domains with great success! 🌟

We are constantly enhancing this tool. Recently, we added the capability to tag CloudWatch Logs. 📝
...
class LogGroupTagger(TaggingInterface):
    ...
    def tag_resources(self, log_group_names, tag_key, tag_value):
        for log_group_name in log_group_names:
            boto_params = {
                'resourceArn': f"{self.log_group_arn_skeleton}{log_group_name}",
                'tags': {
                    tag_key: tag_value
                }
            }
...
def lambda_handler(event, context):
    ...
    function_name = 'your_target_lambda_function_name'
